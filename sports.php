<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <link rel="stylesheet" href="styles.css" />
    <title>ER News Page</title>
  </head>
  <body>
    <navbar class="navbar" id="sports-navbar">
      <div class="navbar-container">
        <div class="logo-container">
          <img
            class="logo-img"
            src="./Images/logo-text-r-letter-market-angle-esports-line-symbol-png-clipart.jpg"
          />
        </div>
        <div class="navbar-link-container">
          <a href="homepage.php" class="navbar-link">Home</a>
          <a href="sports.php" class="navbar-link">Sports</a>
          <a href="innovation.php" class="navbar-link">Innovation</a>
        </div>
        <div class="navbar-buttons">
          <a href="login.php" class="logIn-button">Log In</a>
          <a href="register.php" class="register-button">Register</a>
        </div>
      </div>
    </navbar>
    <hr />
    <section class="hero-section">
      <div class="main-content__wrapper">
        <div class="main-content_left-row">
          <div class="main-article">
            <div class="background-image_container">
              <img class="background-image" src="Images/Vini.webp" />
            </div>
            <div class="main-article_overlay">
              <a href="" class="article-link">
                <h2 class="truncate">When will Vinicius Junior return?</h2>
              </a>
            </div>
          </div>
          <div class="left-row-subarticles-container">
            <div class="subarticle-container">
              <img src="Images/Salah.jpg" alt="" class="subarticle_image" />
              <a href="" class="article-link">
                <p class="truncate">
                  Newcastle 3-3 Liverpool:Four controversial decisions from
                  thrilling Premier League draw analysed
                </p>
              </a>
            </div>
            <div class="subarticle-container">
              <img src="Images/Mbappe.jpg" alt="" class="subarticle_image" />
              <a href="" class="article-link">
                <p class="truncate">
                  Kylian Mbappe apologizes for penalty miss in loss to
                  Athletic:'I take full responsibility'
                </p>
              </a>
            </div>
            <div class="subarticle-container">
              <img
                src="Images/Leverkusen.jpg"
                alt=""
                class="subarticle_image"
              />
              <a href="" class="article-link">
                <p class="truncate">
                  Bayern's Neuer banned for two domestic cup games after
                  receiving first ever red card
                </p>
              </a>
            </div>
          </div>
          <div class="" style="display: none">
            <div class="img-wrapper__main">
              <img class="main-img" src="./Images/SouthKorea.jpg" />
            </div>
            <h2>South Korean president declares Martial Law in South Korea</h2>
            <p>
              South Korea's president shocked the country on Tuesday night when,
              out of the blue, he declared martial law in the Asian democracy
              for the first time in nearly 50 years.
            </p>
          </div>
        </div>
        <div class="main-content_right-row">
          <div class="right-row_article">
            <img
              src="Images/Lebron.jpg"
              alt=""
              class="right-row_article-image"
            />
            <div class="right-row-article_info">
              <h3 class="right-row-article_heading">BASKETBALL</h3>
              <a href="" class="article-link">
                <p class="truncate">
                  Lebron James produced a triple double with 39 points
                </p>
              </a>
            </div>
          </div>
          <div class="right-row_article margin-top">
            <img
              src="Images/Islam.webp"
              alt=""
              class="right-row_article-image"
            />
            <div class="right-row-article_info">
              <h3 class="right-row-article_heading">UFC</h3>
              <a href="" class="article-link">
                <p class="truncate">
                  Islam Makhachev To Defend Lightweight Title again
                </p>
              </a>
            </div>
          </div>
          <div class="right-row_article margin-top">
            <img
              src="Images/Nick.avif"
              alt=""
              class="right-row_article-image"
            />
            <div class="right-row-article_info">
              <h3 class="right-row-article_heading">TENNIS</h3>
              <a href="" class="article-link">
                <p class="truncate">
                  Injury-plagued Kyrgios signals return to Australian Open next
                  month
                </p>
              </a>
            </div>
          </div>
          <div class="right-row_article margin-top">
            <img
              src="Images/uSYK.avif"
              alt=""
              class="right-row_article-image"
            />
            <div class="right-row-article_info">
              <h3 class="right-row-article_heading">BOX</h3>
              <a href="" class="article-link">
                <p class="truncate">Oleksandr Usyk vs Tyson Fury rematch!</p>
              </a>
            </div>
          </div>
          <div class="right-row_article margin-top">
            <img src="Images/Nick.jpg" alt="" class="right-row_article-image" />
            <div class="right-row-article_info">
              <h3 class="right-row-article_heading">FORMULA 1</h3>
              <a href="" class="article-link">
                <p class="truncate">
                  Norris takes pole as front-row lockout puts McLaren on brink
                  of title
                </p>
              </a>
            </div>
          </div>
          <div class="right-row_article margin-top">
            <img src="Images/CS2.jpg" alt="" class="right-row_article-image" />
            <div class="right-row-article_info">
              <h3 class="right-row-article_heading">eSports</h3>
              <a href="" class="article-link">
                <p class="truncate">
                  Perfect World Shanghai Major 2024 of CS2!
                </p>
              </a>
            </div>
          </div>
          <div class="right-row_article margin-top">
            <img src="Images/Ant.jpg" alt="" class="right-row_article-image" />
            <div class="right-row-article_info">
              <h3 class="right-row-article_heading">BASKETBALL</h3>
              <a href="" class="article-link">
                <p class="truncate">
                  Anthony Edwards has 30 points and 9 assists against Warriors
                </p>
              </a>
            </div>
          </div>
        </div>
      </div>
    </section>
    <br />
    <br />
    <hr />
    <br />
    <br />
    <br />
    <footer class="footer">
      <div class="footer-container">
        <div class="about-us__footer">
          <h1 class="footer-h1">About Our Company</h1>
          <br />
          <p>
            Welcome to ER News, your trusted source for breaking news,
            insightful analysis, and in-depth reporting. Our mission is to keep
            you informed, engaged, and empowered by delivering accurate and
            timely news from around the world. At ER News, we are
            committed to journalistic integrity and providing balanced
            perspectives on the stories that matter most. Whether it's politics,
            business, technology, culture, or local events, we aim to bring you
            the facts and context you need to stay ahead.
          </p>
          <p>Copyright 2024 ER Company. All rights reserved.</p>
        </div>
        <div class="our-links__footer">
          <h1 class="footer-h1">Our Links</h1>
          <br />
          <a href="" class="footer-links"> <p>Advertise</p></a>
          <a href="" class="footer-links"><p>Support</p></a>
          <a href="" class="footer-links"><p>Our company</p></a>
          <a href="" class="footer-links"> <p>Contact</p></a>
        </div>
        <div class="social-media-footer-icons">
          <div class="social-media__footer">
            <a href="" class="footer-links"
              ><img class="social-media__pictures" src="./Images/facebook.png"
            /></a>
            <a href="" class="footer-links"
              ><img class="social-media__pictures" src="./Images/twitter.png"
            /></a>
            <a href="" class="footer-links"
              ><img class="social-media__pictures" src="./Images/instagram.png"
            /></a>
            <a href="" class="footer-links"
              ><img class="social-media__pictures" src="./Images/pinterest.png"
            /></a>
          </div>
          <br />
          <div>
            <a href="" class="footer-links"><p>Terms of use</p></a>
            <br />
            <a href="" class="footer-links"><p>Privacy Policy</p></a>
            <br />
            <a href="" class="footer-links"><p>Designed by ER</p></a>
          </div>
        </div>
      </div>
    </footer>
  </body>
</html>
